...
export function* signUp(action) {
  const {email, password, displayName, navigation} = action.payload;
  try {
...
export function* getProfile() {
  const userId = yield select(state => state.userReducer.userId);
  const profileDataResponse = yield FIREBASE.getProfile(userId);
  const profilePostsResponse = yield FIREBASE.getProfilePosts(userId);
  let postsDoc = [];
